 ...
 */
export async function reclaimOffloadData(dataRoot, config, logger) {
    const stats = {
        deletedJsonl: 0,
        deletedRefs: 0,
        deletedMmds: 0,
...
async function reclaimExpiredMmds(agentDirs, cutoffMs, logger) {
    ...
    for (const agentDir of agentDirs) {
        ...
        if (mmdFiles.length <= MIN_KEEP_MMDS)
            continue;
        ...
        for (const meta of fileMetas) {
            if (remaining <= MIN_KEEP_MMDS)
                break;
            if (meta.name === activeMmdFile)
                continue; // never delete active MMD
            if (meta.mtimeMs >= cutoffMs)
