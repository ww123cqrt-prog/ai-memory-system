...
import { normalizeToolCallIdForLookup, getOffloadEntry, populateOffloadLookupMap, isToolResultMessage, extractToolCallId, isOnlyToolUseAssistant, extractAllToolUseIds, isAssistantMessageWithToolUse, isToolUseInAssistant, extractToolUseIdFromAssistant, replaceWithSummary, replaceAssistantToolUseWithSummary, compressNonCurrentToolUseBlocks, getCurrentTaskNodeIds, } from "../l3-helpers.js";
...
function collectHeartbeatToolUseIds(msg) {
    ...
    for (const block of content) {
        if (isHeartbeatToolUseBlock(block) && block.id)
            ids.push(block.id);
    }
    return ids;
}
export function filterHeartbeatMessages(messages, logger) {
    const heartbeatIds = new Set();
    for (const msg of messages) {
        for (const id of collectHeartbeatToolUseIds(msg))
            heartbeatIds.add(id);
    }
    if (heartbeatIds.size === 0)
        return 0;
    let removed = 0;
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (role === "assistant") {
            ...
            for (let j = content.length - 1; j >= 0; j--) {
                if (isHeartbeatToolUseBlock(content[j]))
                    content.splice(j, 1);
            }
...
export function isTokenOverflowError(err) {
    ...
    return (msg.includes("context_length") || msg.includes("context length") ||
...
// ─── Message dump helper ─────────────────────────────────────────────────────
export function dumpMessagesSnapshot(label, messages, logger) {
    const summary = [];
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
        const role = msg.role ?? msg.message?.role ?? msg.type ?? "?";
        const flags = [];
        if (msg._mmdContextMessage)
            flags.push(`mmdCtx=${msg._mmdContextMessage}`);
        if (msg._mmdInjection)
            flags.push("mmdInj");
        if (msg._offloaded)
            flags.push("offloaded");
        ...
        if (typeof content === "string") {
            ...
            const toolUses = content
                .filter((c) => c.type === "tool_use" || c.type === "toolCall")
                .map((c) => `tool_use:${c.name ?? c.id ?? "?"}`);
            const toolResults = content
                .filter((c) => c.type === "tool_result")
                .map((c) => `tool_result:${c.tool_use_id ?? "?"}`);
...
export function createLlmInputL3Handler(stateManager, logger, getContextWindow, pluginConfig, callbacks, backendClient) {
    return async (event) => {
        const _l3Start = Date.now();
        // Skip internal memory-pipeline sessions
        const _sk = stateManager.getLastSessionKey();
        if (typeof _sk === "string" && /memory-.*-session-\d+/.test(_sk))
            return;
        ...
        try {
            ...
            if (historyMessages.length > 0)
                filterHeartbeatMessages(historyMessages, logger);
            ...
            if (historyMessages.length > 0)
                await fastPathReApply(historyMessages, stateManager, logger);
            ...
            logger.debug?.(`[context-offload] L3(llm_input) token snapshot: total=${snap.totalTokens} ` +
            ...
            if (workingTokens >= aggressiveThreshold) {
                ...
                const result = await aggressiveCompressUntilBelowThreshold(historyMessages, offloadMap, currentTaskNodeIds, aggressiveDeleteRatio, stateManager, logger, aggressiveThreshold, countTokens, sysPrompt, promptText);
            ...
            if (workingTokens >= mildThreshold) {
                ...
                if (cascadeResult.replacedCount > 0) {
                    ...
                    const mildStatusUpdates = new Map();
                    for (const id of cascadeResult.replacedToolCallIds) {
            ...
            if (forceEmergency)
                stateManager._forceEmergencyNext = false;
...
export function compressByScoreCascade(messages, offloadMap, currentTaskNodeIds, scanRatio, logger, minCount = MILD_CASCADE_MIN_COUNT, initialScore = MILD_CASCADE_INITIAL_SCORE) {
    ...
    const toolCallIdToResultIdx = new Map();
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (isToolResultMessage(m)) {
            ...
            if (tid) {
                ...
                if (tidNorm !== tid)
                    toolCallIdToResultIdx.set(tidNorm, i);
            }
        }
        if (isAssistantMessageWithToolUse(m)) {
            ...
            for (const tuId of tuIds) {
                ...
                if (tuIdNorm !== tuId)
                    toolCallIdToAssistantIdx.set(tuIdNorm, i);
            }
        }
    }
    let replacedCount = 0;
    ...
    for (let threshold = initialScore; threshold >= MILD_CASCADE_FLOOR_SCORE; threshold--) {
        ...
        for (const c of candidates) {
            ...
            if (c.isAssistantToolUse) {
                ...
                logger.debug?.(`[context-offload] L3-MILD replace: [${c.msgIndex}] ${c.toolCallId} score=${c.score}, ` +
...
function findLastUserMessageIndex(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        const m = messages[i];
        if (m._mmdContextMessage || m._mmdInjection)
            continue;
...
function capDeleteCountForUserMessage(messages, deleteCount) {
    ...
    if (lastUserIdx < 0)
        return deleteCount; // no user msg → nothing to protect
    if (deleteCount <= lastUserIdx)
        return deleteCount; // last user msg is safe beyond the cut
...
function computeAggressiveDeleteCount(messages, remainingTokens, aggressiveThreshold, countTokens, maxDeletable) {
    if (messages.length === 0 || maxDeletable <= 0)
        return 0;
    if (remainingTokens <= aggressiveThreshold)
...
function adjustDeleteCountForToolPairing(messages, initialDeleteCount) {
    if (initialDeleteCount <= 0 || initialDeleteCount >= messages.length)
        return initialDeleteCount;
    let count = initialDeleteCount;
    while (count < messages.length && isToolResultMessage(messages[count]))
        count++;
    return count;
}
/**
 ...
 */
export async function aggressiveCompressUntilBelowThreshold(messages, offloadMap, currentTaskNodeIds, deleteRatio, stateManager, logger, aggressiveThreshold, countTokens, sysPrompt, promptText) {
    ...
    logger.debug?.(`[context-offload] L3-AGGRESSIVE one-shot: deleted=${toDelete.length} msgs, remaining≈${remainingTokens}, msgsLeft=${messages.length}, ` +
...
export function emergencyCompress(messages, targetTokens, countTokens, sysPrompt, promptText, logger) {
    const mmdMsgs = [];
    for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i]._mmdContextMessage || messages[i]._mmdInjection) {
            mmdMsgs.unshift({ msg: messages.splice(i, 1)[0] });
...
function _emergencyTailDelete(messages, targetTokens, currentTokens, deletedToolCallIds, logger) {
    ...
    while (currentTokens - totalTokensDeleted > targetTokens && messages.length > EMERGENCY_MIN_MESSAGES_TO_KEEP) {
        ...
        const groups = [];
        const claimed = new Set(); // indices already in a group
        // Pass 1: Find assistant(tool_use) messages and their paired toolResults
        for (let i = 1; i < messages.length; i++) {
            if (claimed.has(i))
                continue;
            if (i === lastUserIdx)
                continue; // protect last user
            const msg = messages[i];
            const tuIds = extractAllToolUseIds(msg);
        ...
        if (messages.length - best.indices.length < EMERGENCY_MIN_MESSAGES_TO_KEEP)
            break;
...
function _emergencyTruncateOversized(messages, targetTokens, currentTokens, deletedToolCallIds, logger) {
    ...
    for (let i = 0; i < messages.length; i++) {
        if (i === lastUserIdx)
            continue; // protect last user message
        const msg = messages[i];
    ...
    try {
        if (isAssistantTU) {
            ...
            const stubText = `[Tool output truncated for context management. Original ~${bestTokens} tokens, role=${role}${toolCallId ? `, id=${toolCallId}` : ""}]`;
        ...
        if (msg._cachedTokens !== undefined)
            delete msg._cachedTokens;
        if (msg._tokenCount !== undefined)
            delete msg._tokenCount;
        const afterTokens = tiktokenCount(JSON.stringify(msg, jsonReplacer));
        const saved = bestTokens - afterTokens;
        if (toolCallId)
            deletedToolCallIds.push(toolCallId);
        logger.warn(`[context-offload] EMERGENCY truncate-in-place: idx=${bestIdx}, role=${role}, isToolUse=${isAssistantTU}, ` +
...
function _extractContentPreview(msg, maxChars) {
    const content = msg.content ?? msg.message?.content;
    if (typeof content === "string") {
...
export function removeExistingMmdInjections(messages) {
    let removed = 0;
    for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i]._mmdInjection) {
            messages.splice(i, 1);
            removed++;
        }
    }
...
export async function buildHistoryMmdInjection(deletedToolCallIds, offloadMap, offloadEntries, stateManager, logger, countTokens, contextWindow, pluginConfig) {
    ...
    const deletedMmdPrefixes = new Set();
    for (const toolCallId of deletedToolCallIds) {
        ...
        if (entry?.node_id) {
            ...
            if (prefix)
                deletedMmdPrefixes.add(prefix);
        }
    }
    if (deletedMmdPrefixes.size === 0)
        return { injectedMessages: [], totalMmdTokens: 0, mmdTokenBudget, mmdFiles: [] };
    ...
    for (const filename of allMmdFiles) {
        ...
        if (deletedMmdPrefixes.has(filePrefix) && filename !== activeMmd)
            candidateMmds.push(filename);
    }
    if (candidateMmds.length === 0)
        return { injectedMessages: [], totalMmdTokens: 0, mmdTokenBudget, mmdFiles: [] };
...
// ─── Internal helpers ────────────────────────────────────────────────────────
function extractLatestTurn(historyMessages, currentPrompt) {
    ...
    for (let i = historyMessages.length - 1; i >= 0; i--) {
        ...
        if (role === "assistant") {
            ...
            if (text && text.length > 10) {
                lastAssistant = text.slice(0, 600);
                break;
            }
        }
    }
    const parts = [];
    if (currentPrompt)
        parts.push(`[Current User Message]: ${currentPrompt.slice(0, 500)}`);
    if (lastAssistant)
        parts.push(`[Assistant]: ${lastAssistant}`);
...
async function fastPathReApply(messages, stateManager, logger) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (!tid)
            continue;
        const tidNorm = normalizeToolCallIdForLookup(tid);
        if (hasDeleted && (stateManager.deletedOffloadIds.has(tid) || stateManager.deletedOffloadIds.has(tidNorm))) {
            needsWork = true;
            break;
        }
...
//# sourceMappingURL=llm-input-l3.js.map