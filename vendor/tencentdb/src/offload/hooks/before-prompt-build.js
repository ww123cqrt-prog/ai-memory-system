...
import { compressByScoreCascade, aggressiveCompressUntilBelowThreshold, buildHistoryMmdInjection, removeExistingMmdInjections, emergencyCompress, EMERGENCY_MIN_MESSAGES_TO_KEEP, isTokenOverflowError, filterHeartbeatMessages, dumpMessagesSnapshot, } from "./llm-input-l3.js";
export function createBeforePromptBuildHandler(stateManager, logger, getContextWindow, pluginConfig) {
            ...
            for (let i = 0; i < messages.length; i++) {
                ...
                if (tid && hasDeleted && (stateManager.deletedOffloadIds.has(tid) || (tidNorm && stateManager.deletedOffloadIds.has(tidNorm)))) {
                    ...
                    if (isToolResultMessage(msg))
                        deletedToolCallIdsForMmd.push(tid);
                    continue;
                }
                if (hasDeleted && isOnlyToolUseAssistant(msg)) {
                    const tuIds = extractAllToolUseIds(msg);
                    const allDeleted = tuIds.length > 0 && tuIds.every((id) => stateManager.deletedOffloadIds.has(id) || stateManager.deletedOffloadIds.has(normalizeToolCallIdForLookup(id)));
                    if (allDeleted) {
                        indicesToDelete.push(i);
                        continue;
                    }
            ...
            }
            {
                ...
                if ((workingTokens >= emergencyThreshold || forceEmergency) && messages.length > EMERGENCY_MIN_MESSAGES_TO_KEEP) {
                    ...
                    if (emergencyResult.deletedToolCallIds.length > 0) {
                        const emergencyStatusUpdates = new Map();
