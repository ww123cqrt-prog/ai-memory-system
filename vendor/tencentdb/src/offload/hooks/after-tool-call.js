...
import { compressByScoreCascade, aggressiveCompressUntilBelowThreshold, buildHistoryMmdInjection, removeExistingMmdInjections, emergencyCompress, EMERGENCY_MIN_MESSAGES_TO_KEEP, isTokenOverflowError, dumpMessagesSnapshot, } from "./llm-input-l3.js";
import { findActiveMmdInsertionPoint, findHistoryMmdInsertionPoint } from "../mmd-injector.js";
import { buildL3TriggerReport, classifyPatchEffectiveness, reportL3Trigger, recordToolCall, REPORT_TYPE_L3, L3_FIXED_PATCH_COST_TOKENS, } from "../state-reporter.js";
...
function _extractParamsFromMessages(messages, toolCallId) {
    if (!messages || !Array.isArray(messages) || !toolCallId)
        return null;
        ...
        if (Array.isArray(content)) {
            for (const block of content) {
                if ((block.type === "tool_use" || block.type === "toolCall") &&
                    ...
                    if (input && typeof input === "object" && input._offloaded)
                        continue;
                    return input ?? null;
                }
            }
...
export function createAfterToolCallHandler(stateManager, logger, getContextWindow, pluginConfig, backendClient) {
    return async (event, ctx) => {
        ...
        if (_patchStatus.status !== "effective") {
            logger.warn(`[context-offload] after_tool_call patch check: NOT EFFECTIVE (status=${_patchStatus.status}). ` +
        ...
        if (event.messages && Array.isArray(event.messages) && event.messages.length > 0 && !stateManager.cachedLatestTurnMessages) {
            ...
            if (turn)
                stateManager.cachedLatestTurnMessages = turn;
        }
                    ...
                    if (mmdContent) {
                        ...
                        if (existingIdx >= 0) {
                            ...
                            const oldContent = Array.isArray(event.messages[existingIdx].content)
                                ? event.messages[existingIdx].content.map((c) => c.text ?? "").join("")
...
function quickTokenEstimate(messages, stateManager) {
    if (stateManager.lastKnownTotalTokens <= 0)
        return Infinity;
    const newMsgCount = messages.length - stateManager.lastKnownMessageCount;
    if (newMsgCount <= 0)
        return stateManager.lastKnownTotalTokens;
    let newTokensEst = 0;
...
async function checkAndCompressAfterToolCall(event, stateManager, logger, pluginConfig, getContextWindow) {
    try {
        ...
        logger.debug?.(`[context-offload] L3(after_tool_call) token snapshot: tool=${event.toolName} total=${snap.totalTokens} ` +
        ...
        if (snap.totalTokens < mildThreshold)
            return { mildReplacedCount: 0, mildReplacedDetails: [], aggressiveDeletedCount: 0, emergencyTriggered: false, emergencyDeletedCount: 0, snapBefore: snap, snapAfter: snap };
        // L3 compression
        ...
        // Mild
        let _mildResult = { mildReplacedCount: 0, mildReplacedDetails: [] };
