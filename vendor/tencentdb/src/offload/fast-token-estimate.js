 ...
 */
export function fastEstimateTokens(text) {
    if (!text)
        return 0;
    const n = text.length;
    ...
    while (i < n) {
        ...
        if (isKana(cp)) {
            let j = i + 1;
            while (j < n && isKana(text.charCodeAt(j)))
                j++;
            const run = j - i;
            if (run === 1)
                tokens += 1.0;
        ...
        if (isCyrillic(cp)) {
            let j = i + 1;
            while (j < n && isCyrillic(text.charCodeAt(j)))
                j++;
            tokens += (j - i) * 0.55;
            i = j;
            continue;
        ...
        if (isArabic(cp)) {
            let j = i + 1;
            while (j < n && isArabic(text.charCodeAt(j)))
                j++;
            tokens += (j - i) * 0.82;
            i = j;
            continue;
        ...
        if (isGreek(cp)) {
            let j = i + 1;
            while (j < n && isGreek(text.charCodeAt(j)))
                j++;
            tokens += (j - i) * 0.85;
            i = j;
            continue;
        ...
        if (cp >= 0x30 && cp <= 0x39) {
            ...
            if (digits <= 3 && commas === 0 && dots === 0)
                tokens += 1.0;
            else if (commas > 0)
                tokens += commas * 2 + 1.0;
            else if (dots > 0)
                tokens += Math.max(2.0, digits / 3.0 + dots * 1.5);
            else
                tokens += Math.max(1.0, digits / 2.5);
            i = j;
            continue;
        }
 ...
 */
export function fastEstimateMessages(messages, jsonReplacer) {
    let total = 0;
...
//# sourceMappingURL=fast-token-estimate.js.map