...
import { readAllOffloadEntries, readOffloadEntries, markOffloadStatus, DEFAULT_DATA_ROOT, } from "./storage.js";
...
import { normalizeToolCallIdForLookup, getOffloadEntry, populateOffloadLookupMap, isToolResultMessage, extractToolCallId, isOnlyToolUseAssistant, extractAllToolUseIds, isAssistantMessageWithToolUse, replaceWithSummary, replaceAssistantToolUseWithSummary, compressNonCurrentToolUseBlocks, getCurrentTaskNodeIds, } from "./l3-helpers.js";
import { createL3TokenCounter } from "./l3-token-counter.js";
import { compressByScoreCascade, aggressiveCompressUntilBelowThreshold, buildHistoryMmdInjection, removeExistingMmdInjections, emergencyCompress, EMERGENCY_MIN_MESSAGES_TO_KEEP, isTokenOverflowError, } from "./hooks/llm-input-l3.js";
...
import { appendOffloadEntries, rewriteAllOffloadEntries, } from "./storage.js";
...
function parseCreateSkillCommand(prompt) {
    ...
    if (!args)
        return { mmdName: null, skillFocus: null };
...
function _msgFingerprint(msg) {
    ...
    if (typeof raw === "string")
        ...
        content = JSON.stringify(raw).slice(0, 200);
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (role === "user") {
            ...
            if (normalizedCurrent) {
                ...
                if (normalizedText === normalizedCurrent || normalizedText.startsWith(normalizedCurrent) || normalizedCurrent.startsWith(normalizedText))
                    continue;
            }
            // Start a new turn
...
export function registerOffload(api, offloadConfig) {
    ...
    logger.debug?.(`[context-offload] [DIAG] registrationMode=${regMode}, ` +
    ...
    if (offloadConfig.mode === "backend" || offloadConfig.mode === "collect") {
        ...
        if (!offloadConfig.backendUrl) {
            ...
            backendClient = new BackendClient(offloadConfig.backendUrl, logger, offloadConfig.backendApiKey, offloadConfig.backendTimeoutMs, () => _lastActiveSessionKey, () => _resolvedUserId, () => { try {
        ...
        if (!resolvedModelRef) {
            ...
            const mainConfig = api.config;
            const agents = mainConfig?.agents;
        ...
        if (resolvedModelRef) {
            ...
            if (baseUrl && apiKey) {
                backendClient = new LocalLlmClient({ baseUrl, apiKey, model: modelId, temperature: offloadConfig.temperature, timeoutMs: offloadConfig.backendTimeoutMs }, logger);
            }
            else {
                logger.error(`[context-offload] Local LLM mode failed: provider "${providerKey}" not found or missing baseUrl/apiKey in models.providers. ` +
                    `L1/L1.5/L2 disabled.`);
            }
        }
        else {
                ...
                try {
                    const req = {
                        ...
                        toolPairs: chunk.map((p) => ({
                            ...
                            result: typeof p.result === "string" ? sanitizeText(p.result) : p.result,
    ...
    const attemptL15 = async (stateManager, startIndex) => {
        try {
            ...
            const judgment = normalizeJudgment(resp);
            if (!judgment) {
        ...
        try {
            for (const [mmdFile, mmdEntries] of entriesByMmd) {
                ...
                for (let bIdx = 0; bIdx < batches.length; bIdx++) {
                    ...
                    if (changed)
                        await rewriteAllOffloadEntries(stateManager.ctx, allEntries);
                    if (bIdx === 0) {
                    ...
                    try {
                        ...
                        const mmdForBackfill = typeof mmdAfterWrite === "string" && mmdAfterWrite.trim().length > 0
                            ? mmdAfterWrite
    ...
    const createSkillWithBackend = async (stateManager, skillCommand) => {
        if (!backendClient || !skillCommand.mmdName)
            return null;
        try {
    ...
    const getContextWindow = () => {
        try {
            ...
            const defaultModel = typeof defaults?.model === "string"
                ? defaults.model
                : (typeof defaults?.model === "object" && typeof defaults?.model?.primary === "string")
            ...
            if (models?.contextWindow && typeof models.contextWindow === "number")
    ...
    const armL2Poll = () => {
        if (_l2PollHandle !== null)
            return;
        if (_l2FirstNotifyAt === null)
            _l2FirstNotifyAt = Date.now();
        const tick = async () => {
            _l2PollHandle = null;
    ...
    const notifyL2NewNullEntries = (newNullCount) => {
        if (!_lastActiveMgr || newNullCount <= 0)
            return;
        armL2Poll();
    };
    ...
    const _trackedOn = (hookName, handler) => {
        ...
        if (typeof api.on === "function") {
            api.on(hookName, (...args) => {
                if (_contextEngineRejected)
                    return; // slot not acquired — all offload disabled
                return handler(...args);
            });
        }
        else {
    ...
    _trackedOn("llm_output", async (event, ctx) => {
        ...
        if (pendingCount > 0) {
            logger.debug?.(`[context-offload] llm_output: ${pendingCount} pending tool pairs (will be flushed at next llm_input or after_tool_call batch)`);
        }
    });
    ...
    _trackedOn("llm_input", async (event, _ctx) => {
        ...
        if (isInternalMemorySession(_ctx?.sessionKey))
            return;
    ...
    _trackedOn("before_agent_start", async (event, ctx) => {
        if (isInternalMemorySession(ctx?.sessionKey))
            return;
        const sk = ctx?.sessionKey;
    ...
    if (offloadConfig.mode === "collect") {
        ...
        if (_lastActiveMgr)
            _lastActiveMgr.l15Settled = true;
        ...
        const scheduleReclaim = (delayMs) => {
            _reclaimTimer = setTimeout(async () => {
                try {
                    ...
                    logger.debug?.(`[context-offload] Reclaim done: jsonl=${stats.deletedJsonl}, refs=${stats.deletedRefs}, ` +
    ...
    async ingest(params) {
        ...
        if (role === "toolResult" || role === "tool") {
            ...
            if (toolCallId) {
                ...
                if (mgr)
                    mgr.processedToolCallIds.add(toolCallId);
        ...
        try {
            ...
            if (!prompt || typeof prompt !== "string" || prompt.length === 0) {
                ...
                if (promptHash === lastHash) {
                    ...
                    this._judgeL15(stateManager, { prompt, messages: workMessages }, { sessionKey: stateManager.getLastSessionKey() }).catch((err) => {
            ...
            if (hasConfirmed || hasDeleted) {
                ...
                if (indicesToDelete.length > 0) {
                    for (let k = indicesToDelete.length - 1; k >= 0; k--)
                        workMessages.splice(indicesToDelete[k], 1);
                }
            }
                    ...
                    if (result.allDeletedToolCallIds.length > 0) {
                        ...
                        if (mmdInj.injectedMessages.length > 0) {
                            ...
                            for (let ii = 0; ii < mmdInj.injectedMessages.length; ii++) {
                                ...
                                let ic = "";
                                if (typeof im.content === "string")
                                    ic = im.content;
                                else if (Array.isArray(im.content))
                                    ic = im.content.map((c) => typeof c === "string" ? c : (c.text ?? "")).join(" ");
            ...
            if (workingTokens >= mildThreshold) {
                ...
                if (cascadeResult.replacedCount > 0) {
                    for (const id of cascadeResult.replacedToolCallIds)
                        stateManager.confirmedOffloadIds.add(id);
                    const mildUpdates = new Map();
                    for (const id of cascadeResult.replacedToolCallIds)
                        mildUpdates.set(id, true);
            ...
            if (pendingCount > 0) {
                ...
                this._flushL1(stateManager, "afterTurn_flush").then(async () => {
                    try {
                        ...
                        const nullCount = allEntries.filter((e) => e.node_id === null).length;
                        if (nullCount > 0)
                            this._notifyL2NewNullEntries(nullCount);
                    }
                    catch { /* ignore */ }
                }).catch((err) => {
