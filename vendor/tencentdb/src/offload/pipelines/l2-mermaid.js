...
import { readAllOffloadEntries, rewriteAllOffloadEntries, } from "../storage.js";
...
function pickMmdDerivedFallbackNodeId(mmdText, mmdPrefix) {
    ...
    if (ids.length === 0)
        return null;
    const prefix = typeof mmdPrefix === "string" && /^\d{3}$/.test(mmdPrefix)
        ? `${mmdPrefix}-`
        : null;
...
export async function checkL2Trigger(stateManager, pluginConfig, logger) {
    ...
    const timeNeedsNewOffload = pluginConfig?.l2TimeTriggerRequiresNewOffload ??
        PLUGIN_DEFAULTS.l2TimeTriggerRequiresNewOffload;
    const waitRetrySeconds = pluginConfig?.l2WaitRetrySeconds ??
        PLUGIN_DEFAULTS.l2WaitRetrySeconds;
    const emptyResult = { shouldTrigger: false, reason: "", entriesByMmd: new Map() };
    ...
    const entriesByMmd = new Map();
    let eligibleNullCount = 0;
    for (let i = 0; i < allEntries.length; i++) {
        const entry = allEntries[i];
        if (isHeartbeatEntry(entry))
            continue;
        if (entry.node_id !== null && entry.node_id !== "wait")
            continue;
        ...
        if (entry.node_id === "wait") {
            ...
            if (tsIso) {
                ...
                if (!Number.isNaN(tsMs) && (nowMs - tsMs) / 1000 < waitRetrySeconds)
                    continue;
            }
        }
        ...
        if (!boundary)
            continue; // no boundary coverage → skip
        if (boundary.result !== "long")
            continue; // short task → skip
        if (!boundary.targetMmd)
            continue; // no target mmd → skip
        ...
        if (entry.tool_call_id && bucket.some((e) => e.tool_call_id === entry.tool_call_id))
            continue;
        bucket.push(entry);
    }
...
export async function backfillNodeIds(ctx, nodeMapping, waitIds, logger, options) {
...
//# sourceMappingURL=l2-mermaid.js.map