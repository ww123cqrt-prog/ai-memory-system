...
function extractL3TriggerSource(stage) {
    if (!stage || !stage.startsWith("L3"))
        return undefined;
    if (stage.includes("after_tool_call"))
        return "after_tool_call";
    if (stage.includes("llm_input"))
        return "llm_input";
    if (stage.includes("before_prompt"))
        return "before_prompt_reapply";
    return "L3_unknown";
}
...
function durationBucketTag(ms) {
    ...
    if (ms < 1000)
        return "duration:<1s";
    if (ms < 5000)
        return "duration:1-5s";
    if (ms < 15000)
        return "duration:5-15s";
    if (ms < 30000)
        return "duration:15-30s";
    return "duration:>30s";
}
...
function getOpikConfigFromOpenClawConfig(config) {
    ...
    const apiUrl = typeof opikCfg?.apiUrl === "string" ? opikCfg.apiUrl : getEnv("OPIK_URL_OVERRIDE");
    const apiKey = typeof opikCfg?.apiKey === "string" ? opikCfg.apiKey : getEnv("OPIK_API_KEY");
    const workspaceName = typeof opikCfg?.workspaceName === "string" && opikCfg.workspaceName.trim()
        ? opikCfg.workspaceName
        : getEnv("OPIK_WORKSPACE") ?? "default";
    const projectName = typeof opikCfg?.projectName === "string" && opikCfg.projectName.trim()
        ? opikCfg.projectName
        : getEnv("OPIK_PROJECT_NAME") ?? "openclaw";
    return { enabled, apiUrl, apiKey, workspaceName, projectName };
}
export function initOffloadOpikTracer(openClawConfig, logger) {
    if (tracerInitTried)
        return;
...
//# sourceMappingURL=opik-tracer.js.map