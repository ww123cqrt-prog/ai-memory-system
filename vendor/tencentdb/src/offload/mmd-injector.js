 ...
 */
import { readMmd } from "./storage.js";
 ...
 */
export async function injectMmdIntoMessages(messages, stateManager, logger, getContextWindow, pluginConfig, options) {
    ...
    const contextWindow = typeof getContextWindow === "function"
        ? getContextWindow()
    ...
    if (totalMmdTokens > 0) {
        const mmdCount = messages.filter((m) => m[MMD_MESSAGE_MARKER] === "active" || m._mmdInjection).length;
 ...
 */
export async function maybeUpdateMmdInMessages(messages, stateManager, logger, getContextWindow, pluginConfig) {
    const injectionReady = stateManager.isMmdInjectionReady();
    const activeMmdFile = stateManager.getActiveMmdFile();
    logger.debug?.(`[context-offload] mmd-injector maybeUpdate: injectionReady=${injectionReady}, activeMmdFile=${activeMmdFile ?? "null"}, msgs=${messages.length}`);
    if (!injectionReady)
        return false;
    if (!activeMmdFile)
        return false;
    ...
    try {
        ...
        logger.debug?.(`[context-offload] mmd-injector maybeUpdate: readMmd result=${mmdContent ? `${mmdContent.length} chars` : "null"}`);
    }
    catch (e) {
    ...
    if (newFp === lastFp)
        return false;
    logger.debug?.(`[context-offload] mmd-injector: MMD updated (${activeMmdFile}), refreshing in-loop`);
 ...
 */
export function findActiveMmdInsertionPoint(messages) {
...
function adjustForToolCallPair(messages, idx) {
    ...
    while (cur > 0 && cur < messages.length) {
        ...
        if (msg[MMD_MESSAGE_MARKER] || msg._mmdInjection) {
            cur--;
            continue;
        }
 ...
 */
export function findHistoryMmdInsertionPoint(messages) {
    // If there's an existing active MMD, insert just before it
    for (let i = 0; i < messages.length; i++) {
        if (messages[i][MMD_MESSAGE_MARKER] === "active")
            return i;
    }
...
//# sourceMappingURL=mmd-injector.js.map