 ...
 */
export function normalizeToolCallIdForLookup(id) {
    return id.replace(/_/g, "");
}
export function getOffloadEntry(map, toolCallId) {
...
export function populateOffloadLookupMap(map, entries) {
...
export function extractToolCallId(msg) {
    if (msg.type === "message") {
        const message = msg.message;
...
export function isAssistantMessageWithToolUse(msg) {
...
export function isToolUseInAssistant(msg) {
    return isAssistantMessageWithToolUse(msg);
}
/** Extract tool_use ID from an assistant message (first tool_use block) */
export function extractToolUseIdFromAssistant(msg) {
    const content = getMessageContent(msg);
    if (!Array.isArray(content))
        return null;
    for (const block of content) {
        const b = block;
        if (isToolUseBlock(b) && b.id)
            return b.id;
    }
    return null;
}
 ...
 */
export function isOnlyToolUseAssistant(msg) {
...
export function extractAllToolUseIds(msg) {
    const content = getMessageContent(msg);
    if (!Array.isArray(content))
        return [];
    const ids = [];
...
export function compactToolCall(toolCall) {
    if (!toolCall || typeof toolCall !== "string")
        return toolCall ?? "";
    if (toolCall.length <= COMPACT_TOOL_CALL_MAX_TOTAL)
        return toolCall;
 ...
 */
export function replaceAssistantToolUseWithSummary(msg, entries) {
    const content = getMessageContent(msg);
    if (!Array.isArray(content))
        return;
    ...
    for (const block of content) {
        ...
        if (!isToolUseBlock(b))
            continue;
        const entry = (b.id && entryById.get(b.id)) ??
...
export function replaceWithSummary(msg, entry) {
    ...
    const extractLength = (content) => {
        ...
        if (Array.isArray(content))
            return content.reduce((acc, c) => acc + (typeof c === "string" ? c.length : (c.text?.length ?? 0)), 0);
        return 0;
    };
    if (msg.type === "message") {
        const message = msg.message;
 ...
 */
export function compressNonCurrentToolUseBlocks(msg, offloadMap, currentTaskNodeIds, replacedIds) {
    const content = getMessageContent(msg);
    if (!Array.isArray(content))
        return;
    for (const block of content) {
        ...
        if (!entry)
            continue;
        const idInReplacedIds = replacedIds &&
...
export async function getCurrentTaskNodeIds(stateManager) {
...
//# sourceMappingURL=l3-helpers.js.map