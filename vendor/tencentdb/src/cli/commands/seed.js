 ...
 */
export function registerSeedCommand(parent, ctx) {
    parent
        .command("seed")
...
async function runSeedCommand(opts, ctx) {
    ...
    if (needsTimestampConfirmation) {
        if (opts.yes) {
            ...
            const confirmed = await askConfirmation("All messages have no timestamp. Use current time for each conversation round? [y/N] ");
            if (!confirmed) {
                console.log("Aborted.");
                process.exit(0);
            }
    ...
    const summary = await executeSeed(input, {
        ...
        onProgress: (progress) => {
            ...
            process.stdout.write(`\r  [${progress.currentRound}/${progress.totalRounds}] ${pct}% ` +
