...
function createConsoleLogger() {
    return {
        debug: (msg) => console.debug(`${TAG} ${msg}`),
        info: (msg) => console.info(`${TAG} ${msg}`),
...
export class TdaiGateway {
    ...
    logSecurityPosture() {
        ...
        this.logger.info(`Security posture: auth=${authOn ? "ENABLED (Bearer)" : "disabled"} ` +
            `host=${host} cors=${corsOrigins.length === 0 ? "no-headers" : corsOrigins.includes("*") ? "wildcard(*)" : `allowlist(${corsOrigins.length})`}`);
        if (!authOn) {
            this.logger.warn("TDAI_GATEWAY_API_KEY is NOT set — all routes except GET /health are " +
        ...
        if (!loopback && !authOn) {
            this.logger.warn(`Gateway is bound to ${host} (non-loopback) WITHOUT an API key. ` +
        ...
        if (corsOrigins.includes("*")) {
            this.logger.warn("CORS allow-list contains '*' — every browser origin can call this " +
    ...
    checkAuth(req, res) {
        ...
        if (!expected)
            return true; // auth disabled — default behaviour
    ...
    applyCorsHeaders(req, res) {
        ...
        if (allow.length === 0)
            return; // strict default — no headers
        if (allow.includes("*")) {
    ...
    async handleSearchMemories(req, res) {
        const body = await parseJsonBody(req);
        if (!body.query) {
    ...
    async handleSearchConversations(req, res) {
        const body = await parseJsonBody(req);
        if (!body.query) {
            sendError(res, 400, "Missing required field: query");
            return;
    ...
    async handleSeed(req, res) {
        ...
        this.logger.info(`Seed request: ${input.sessions.length} session(s), ` +
        ...
        const baseConfig = this.config.memory;
        let pluginConfig = {
            ...baseConfig,
            llm: {
                enabled: true,
        ...
        if (body.config_override) {
            for (const key of Object.keys(body.config_override)) {
                ...
                if (baseVal && typeof baseVal === "object" && !Array.isArray(baseVal) &&
                    ...
                    pluginConfig[key] = { ...baseVal, ...overVal };
                }
                else {
        ...
        const summary = await executeSeed(input, {
            ...
            logger: this.logger,
            onProgress: (progress) => {
                this.logger.debug?.(`Seed progress: [${progress.currentRound}/${progress.totalRounds}] ` +
                    `session=${progress.sessionKey} stage=${progress.stage}`);
            },
        });
        this.logger.info(`Seed complete: sessions=${summary.sessionsProcessed}, rounds=${summary.roundsProcessed}, ` +
