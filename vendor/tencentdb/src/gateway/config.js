...
export function loadGatewayConfig(overrides) {
    ...
    if (configPath) {
        try {
            ...
            fileConfig = expandEnvVars(fileConfig);
        }
        catch {
    ...
    const memory = parseMemoryConfig(memoryRaw);
    const base = {
    ...
    try {
        if (!fs.existsSync(newDefault)) {
            ...
            if (fs.existsSync(legacy)) {
                ...
                process.stderr.write(`[tdai-gateway] DEPRECATED: using legacy data dir ${legacy}; ` +
 ...
 */
function resolveCorsOrigins(serverConfig) {
