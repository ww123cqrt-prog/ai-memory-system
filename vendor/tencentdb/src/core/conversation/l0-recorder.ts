...
export async function readConversationMessages(
  ...
  if (limit != null && limit > 0 && allMessages.length > limit) {
    logger?.debug?.(
      `${TAG} readConversationMessages: truncating ${allMessages.length} → ${limit} (oldest page)`,
    );
...
export async function readConversationMessagesGroupedBySessionId(
  ...
  if (limit != null && limit > 0 && allMessages.length > limit) {
    logger?.debug?.(
      `${TAG} readConversationMessagesGroupedBySessionId: truncating ${allMessages.length} → ${limit} (oldest page)`,
    );
